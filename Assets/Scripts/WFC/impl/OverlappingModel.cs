...
public class OverlappingModel : Model
{
    ...
    public OverlappingModel(Texture2D texture, int N, int width, int height, bool periodicInput, bool periodic, int symmetry, bool ground, Heuristic heuristic)
        : base(width, height, N, periodic, heuristic)
    {
        Debug.Log("overlapping model");
        int[] bitmap = WFC.textureToIntArray(texture);
        var SX = texture.width;
        var SY = texture.height;
